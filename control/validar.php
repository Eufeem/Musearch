   ...
   <div class="container" id="principal">
       <div class="row">
           <div class="col-md-9">
               ...
               <h2 class="text-center hdr">Musearch</h2>aazssa
               <p class="justificado">
...
<?php
    ...
    if (isset($_POST['login'])) {
        $user = $_POST['nombre_admi'];
        $pw = $_POST['contrasena_admin'];
        $log = mysqli_query($con, "SELECT * FROM administrador WHERE nombre_admi = '$user'
                and contrasena_admin = '$pw' ") or die ("Problemas en la consulta del login");

        if (mysqli_num_rows($log)>0) {
            $row = mysqli_fetch_array($log);
            $_SESSION["user"] = $row['nombre_admi'];
            echo '<script> window.location="../view/vistaAdministrador.php"; </script>';
            echo '<script>alert("Bienvenido Administrador);</script>';
        } else {
            echo '<script>alert("El usuario o contraseña que ingresaste son incorrectos");</script>';
